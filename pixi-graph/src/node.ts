...
export class PixiNode extends TypedEmitter<PixiNodeEvents> {
  ...
  private createNode() {
    ...
    nodeGfx.on('rightdown', (event: InteractionEvent) => { this.emit('rightdown', event.data.originalEvent as MouseEvent); event.stopPropagation() });
    nodeGfx.on('rightup', (event: InteractionEvent) => { this.emit('rightup', event.data.originalEvent as MouseEvent); event.stopPropagation() });
    createNode(nodeGfx);
    return nodeGfx;
  }
  ...
  private createNodeLabel() {
    ...
    nodeLabelGfx.on('rightdown', (event: InteractionEvent) => this.emit('rightdown', event.data.originalEvent as MouseEvent));
    nodeLabelGfx.on('rightup', (event: InteractionEvent) => this.emit('rightup', event.data.originalEvent as MouseEvent));
