...
export class PixiEdge extends TypedEmitter<PixiEdgeEvents> {
  ...
  createEdge() {
    ...
    edgeGfx.on('rightdown', (event: InteractionEvent) => { this.emit('rightdown', event.data.originalEvent as MouseEvent); event.stopPropagation() });
    edgeGfx.on('rightup', (event: InteractionEvent) => { this.emit('rightup', event.data.originalEvent as MouseEvent); event.stopPropagation() });
    createEdge(edgeGfx);
    return edgeGfx;
  }
